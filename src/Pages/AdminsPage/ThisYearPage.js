...
useEffect(() => {
      searchParams.append("SortBy", "ThisYear");
...
useEffect(()=>{
  fetch(`https://localhost:7191/Cart/money-amount-in-a-year?year=${year}`)
  .then(response => response.json())
...
const handleClick = (event) => {
 ...
 fetch(`https://localhost:7191/Cart/export-this-year`, {
    method: 'GET',
      headers: {
          'Content-Type': 'application/json',
      },
  ...
  return isAdmin ? (
    <ThemeProvider theme={mdTheme}>
      ...
      <div className="ms-auto mt-5">
        ...
        <Title>Doanh Thu Năm {year}</Title>
        <Table size="small">
        <TableHead>
          <TableRow>
            <TableCell>Ngày</TableCell>
