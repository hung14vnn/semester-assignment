...
export default function PrevMonthPage() {
...
useEffect(() => {
      searchParams.append("SortBy", "PrevMonth");
...
useEffect(()=>{
  fetch(`https://localhost:7191/Cart/money-amount-in-a-month?month=${month-1}&year=${year}`)
  .then(response => response.json())
...
const handleClick = (event) => {
 ...
 fetch(`https://localhost:7191/Cart/export-prev-month`, {
    method: 'GET',
      headers: {
          'Content-Type': 'application/json',
      },
  ...
  .then(blob => {
      ...
      a.href = url;
      a.download = `Bao-cao-thang-${month-1}-nam-${year}.xlsx`;
      a.click();
  ...
  return isAdmin ? (
    <ThemeProvider theme={mdTheme}>
      ...
      <div className="ms-auto mt-5">
                 ...
                 {/* print out the avanue of the month */}
        <Title>Doanh Thu Tháng {month-1}/{year}</Title>
        <Table size="small">
        <TableHead>
          <TableRow>
            <TableCell>Ngày</TableCell>
        ...
        <TableBody>
            <TableRow>
              ...
              <TableCell>{new Date(year, month-1, 0).toLocaleDateString('en-GB')}</TableCell>
              <TableCell>{`${dataDeposit}Đ`}</TableCell>
            </TableRow>
        </TableBody>
      </Table>
    </React.Fragment>
